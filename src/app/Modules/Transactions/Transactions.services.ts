...
const updateTransactionIntoDB = async (id: string) => {
  const updateTransactionInfo = await TransactionsModel.findByIdAndUpdate(
    id,
    { status: 'Complete' },
    {
      new: true,
  ...
  const updateListingInfo = await ListingsModel.findByIdAndUpdate(
    updateTransactionInfo?.itemId?._id,
    { status: 'Sold' },
    {
      new: true,
    },
...
const getAllPurchasesFromDB = async (id: string) => {
  const purchaseData = await TransactionsModel.find({ buyerId: id })
    .populate('buyerId')
    .populate('sellerId')
    .populate('itemId');
  if (!purchaseData) {
...
const getAllSalesFromDB = async (id: string) => {
  const salesData = await TransactionsModel.find({ sellerId: id })
    .populate('buyerId')
    .populate('sellerId')
    .populate('itemId');
  if (!salesData) {
